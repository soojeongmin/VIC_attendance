const SPREADSHEET_ID = "1O6ZPv9yi7-gZJ8HR_56cJFKxnlbQ697vlN7QvoMh2uQ";
const ABSENCE_SHEET_ID = "11EeOofOHbRa6vn26pBptDx_FSFDUdEeo-TglLgzD3mI"; // 2학년부 사전 결석 정보 시트
const DEVELOPER_EMAIL = "kmhee1983@cnsa.hs.kr"; // 2학년부 담당 교사 이메일
...
function getClassroomList() {
  return ["3A", "3B", "3C", "3D", "C306", "C307", "C309"];
}
...
function getStudentMap() {
  ...
  for (let i = 1; i < placementData.length; i++) {
    ...
    studentMap[movedSeat] = {
      ...
      email,
      hr: "", // HR 정보 없음
      classroom: movedSeat.split("-")[0], // C309-001 → C309
      moveDate,
      movedSeat: movedSeat
    };
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (rowDate === date && rowStudentId === String(studentId)) {
      violations.push({
        ...
        note: data[i][7],
        recordTime: data[i][11] ? Utilities.formatDate(new Date(data[i][11]), "Asia/Seoul", "HH:mm") : ""
      });
    }
  }
...
function saveAttendanceBulk(entries) {
  ...
  try {
    ...
    // 발전실 여부 확인 (2학년부: C309만 발전실)
    const classroom = entries[0].classroom;
    const isDevRoom = ["C309"].includes(classroom);
...
function searchStudentByStudentId(studentId) {
  ...
  for (let i = 1; i < dataValues.length; i++) {
    ...
    if (String(stdId) === searchId) {
      // 좌석번호에서 교실명 추출 (C306-024 → C306)
      let actualClassroom = classroom;
      if (seat && seat.includes('-')) {
        actualClassroom = seat.split('-')[0];
      }
...
function getPreAbsenceData() {
  // ABSENCE_SHEET_ID가 설정되지 않은 경우 빈 객체 반환
  if (!ABSENCE_SHEET_ID) {
    return { ET: {}, EP1: {} };
  }
...
function getStudentPreAbsence(studentId, timeSlot) {
  // ABSENCE_SHEET_ID가 설정되지 않은 경우 null 반환
  if (!ABSENCE_SHEET_ID) {
    return null;
  }
...
/**
 * 학번에서 반 번호 추출 (2학년용)
 * 20103 → 1, 21111 → 11, 20823 → 8
 */
function getClassFromStudentId(studentId) {
  const idStr = String(studentId);
  if (idStr.length !== 5 || !idStr.startsWith('2')) return null;
...
function sendDailyClassReport() {
  ...
  for (let classNum = 1; classNum <= 12; classNum++) {
    ...
    const teacherEmail = teacherEmails[classNum];
    if (!teacherEmail) {
    ...
    // 이메일 본문 생성
    const subject = `[2학년 ${classNum}반] ${yesterdayDisplay} 면학실 일일 보고`;

    let body = `📋 2학년 ${classNum}반 면학실 일일 보고서
📅 날짜: ${yesterdayDisplay}
`;
...
`;
  ...
  if (chiefEmail) {
    const subject = `[2학년부] ${yesterdayDisplay} 면학실 전체 현황`;
